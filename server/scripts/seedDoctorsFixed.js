...
const seedDoctors = async () => {
  try {
    ...
    // Insert doctors one by one to catch any validation errors
    console.log('Inserting doctors one by one...');
    let insertedCount = 0;
    
    for (let i = 0; i < sampleDoctors.length; i++) {
      try {
        const doctor = new Doctor(sampleDoctors[i]);
        await doctor.save();
        insertedCount++;
        console.log(`✅ Inserted doctor ${i + 1}: ${doctor.name}`);
      } catch (error) {
        console.error(`❌ Error inserting doctor ${i + 1} (${sampleDoctors[i].name}):`, error.message);
      }
    }
    
    console.log(`Successfully inserted ${insertedCount} doctors`);
